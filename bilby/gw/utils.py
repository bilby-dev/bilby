...
@xp_wrap
def frequency_domain_whiten(
    frequency_series, amplitude_spectral_density, frequency_mask, duration, *, xp=None
):
    """
    ...
    return xp.nan_to_num(whitened) * frequency_mask * (4 / duration)**0.5


@xp_wrap
def whiten_and_crop(
    frequency_series, amplitude_spectral_density, frequency_mask, time_mask, duration, *, xp=None
):
    """
    ...
    """
    whitened = frequency_series / amplitude_spectral_density
    initial_white = xp.nan_to_num(whitened) * frequency_mask
    time_series = xp.fft.irfft(initial_white)
    cropped_time_series = time_series[time_mask]
    cropped_white = xp.fft.rfft(cropped_time_series)
