...
def gwsignal_binary_black_hole(
    frequency_array,
    mass_1,
    mass_2,
    luminosity_distance,
    ...
    **kwargs,
    ...
    if waveform_approximant not in ["SEOBNRv5HM", "SEOBNRv5PHM"]:
        if waveform_approximant == "IMRPhenomXPHM":
            logger.warning(
                "The new waveform interface is unreviewed for this model" + "and it is only intended for testing."
            )
        else:
    ...
    if mode_array is not None:
        try:
            ...
            raise ValueError(
                f"Unable to convert mode_array elements to tuples of ints. mode_array: {mode_array}, Error: {e}"
            ) from e
    ...
    try:
        ...
        if not catch_waveform_errors:
            ...
            EDOM = ("Internal function call failed: Input domain error" in e.args[0]) or "Input domain error" in e.args[
                0
            ]
            if EDOM:
    ...
    if len(hplus) > len(frequency_array):
        logger.debug(
            ...
            + f"({len(hplus)} vs {len(frequency_array)}), "
...
def lal_eccentric_binary_black_hole_no_spins(
    frequency_array, mass_1, mass_2, eccentricity, luminosity_distance, theta_jn, phase, **kwargs
):
...
def set_waveform_dictionary(waveform_kwargs, lambda_1=0, lambda_2=0):
    ...
    for key in ["pn_spin_order", "pn_tidal_order", "pn_phase_order", "pn_amplitude_order"]:
...
def _base_lal_cbc_fd_waveform(
    ...
    **waveform_kwargs,
    ...
    if pn_amplitude_order != 0:
        start_frequency = lalsim.SimInspiralfLow2fStart(float(minimum_frequency), int(pn_amplitude_order), approximant)
    else:
        start_frequency = minimum_frequency
    ...
    if len(hplus.data.data) > len(frequency_array):
        logger.debug(
            "LALsim waveform longer than bilby's `frequency_array`"
    ...
    **waveform_arguments,
    ...
    if "frequency_nodes" not in waveform_arguments:
        ...
        frequency_nodes_combined = np.hstack(
            (waveform_arguments.pop("frequency_nodes_linear"), waveform_arguments.pop("frequency_nodes_quadratic"))
        )
        frequency_nodes_unique, original_indices = np.unique(frequency_nodes_combined, return_inverse=True)
...
def sinegaussian(frequency_array, hrss, Q, frequency, **kwargs):
    ...
    h_plus = (
        (hrss / np.sqrt(temp * (1 + np.exp(-(Q**2)))))
        * ((np.sqrt(np.pi) * tau) / 2.0)
        * (np.exp(-(fm**2) * np.pi**2 * tau**2) + np.exp(-(fp**2) * np.pi**2 * tau**2))
    )
...
def supernova_pca_model(
    ...
    strain = np.sum([coeff * principal_components[:, ii] for ii, coeff in enumerate(coefficients)], axis=0)
...
primary_spin_and_q = {"a_1", "chi_1", "mass_ratio"}
tidal = {"lambda_1", "lambda_2", "lambda_tilde", "delta_lambda_tilde"}
phase = {
    "phase",
    "delta_phase",
}
