...
class PowerSpectralDensity:
    def __init__(self, frequency_array=None, psd_array=None, asd_array=None, psd_file=None, asd_file=None):
        """
        ...
        """
        self._cache = dict(frequency_array=np.array([]), psd_array=None, asd_array=None)
    ...
    def __repr__(self):
        if self.asd_file is not None or self.psd_file is not None:
            return self.__class__.__name__ + f"(psd_file='{self.psd_file}', asd_file='{self.asd_file}')"
        else:
            return (
                self.__class__.__name__
                + f"(frequency_array={self.frequency_array}, psd_array={self.psd_array}, asd_array={self.asd_array})"
            )
    ...
    def from_channel_name(
        ...
        strain.set_from_channel_name(
            channel, duration=psd_duration, start_time=psd_start_time, sampling_frequency=sampling_frequency
        )
        frequency_array, psd_array = strain.create_power_spectral_density(
            fft_length=fft_length,
            name=name,
    ...
    @staticmethod
    def from_aligo():
        logger.info("No power spectral density provided, using aLIGO,zero detuning, high power.")
    ...
    def __interpolate_power_spectral_density(self):
        ...
        """
        self.__power_spectral_density_interpolated = interp1d(
            self.frequency_array, self.psd_array, bounds_error=False, fill_value=np.inf
        )
    ...
    def __validate_file_name(file):
        ...
        if file is None:
            ...
            file_in_default_directory = os.path.join(os.path.dirname(__file__), "noise_curves", file)
            if os.path.isfile(file_in_default_directory):
                ...
                raise ValueError(f"Unable to locate PSD file {file} locally or in the default dir")
        return file
