...
from ...core.utils import create_frequency_series, logger, radius_of_earth, speed_of_light
...
class ROQGravitationalWaveTransient(GravitationalWaveTransient):
    ...
    def __init__(
            ...
            if self.roq_params is None:
                ...
                if is_hdf5_quadratic:
                    if self.roq_params is None:
                        ...
                        self.roq_params["seglen"] = min(self.roq_params["seglen"], quadratic_matrix["duration_s"][()])
        ...
        if self.number_of_bases_linear == 1 and self.number_of_bases_quadratic == 1:
            frequency_nodes, linear_indices, quadratic_indices = self._unique_frequency_nodes_and_inverse[0][0]
    ...
    def _verify_numbers_of_prior_ranges_and_frequency_nodes(self, basis_type):
        ...
        try:
            ...
            raise AttributeError(f'For the use of multiple {basis_type} ROQ bases, weights should contain "{key}".')
        else:
            for param_name in prior_ranges:
                if len(prior_ranges[param_name]) != number_of_bases:
                    raise ValueError(
                        f'The number of prior ranges for "{param_name}" does not match the number of {basis_type} bases'
                    )
        ...
        for param_name, prior_ranges_of_this_param in prior_ranges.items():
            ...
            if prior_minimum < basis_minimum:
                raise BilbyROQParamsRangeError(
                    f"Prior minimum of {param_name} {prior_minimum} less than ROQ basis bound {basis_minimum}"
                )
            ...
            if prior_maximum > basis_maximum:
                raise BilbyROQParamsRangeError(
                    f"Prior maximum of {param_name} {prior_maximum} greater than ROQ basis bound {basis_maximum}"
                )
        ...
        for idx_linear in range(self.number_of_bases_linear):
            ...
            for idx_quadratic in range(self.number_of_bases_quadratic):
                ...
                frequency_nodes_unique, original_indices = np.unique(
                    np.hstack((frequency_nodes_linear, frequency_nodes_quadratic)), return_inverse=True
                )
                linear_indices = original_indices[:size_linear]
                quadratic_indices = original_indices[size_linear:]
                tmp.append((frequency_nodes_unique, linear_indices, quadratic_indices))
    ...
    def _setup_time_marginalization(self):
        ...
        logger.info(f"delta tc for time marginalization = {self._delta_tc} seconds.")
        self._times = tcmin + self._delta_tc / 2.0 + np.arange(number_of_time_samples) * self._delta_tc
        self._beam_pattern_reference_time = (tcmin + tcmax) / 2.0
    ...
    def _parse_basis(basis, basis_type):
        ...
        if isinstance(basis, str):
            ...
            if format == "npy":
                ...
                raise OSError(f"Format {format} not recognized.")
    ...
    def _select_prior_ranges(self, prior_ranges):
        ...
        for param_name in param_names:
            ...
            in_prior_range *= (prior_ranges_of_this_param[:, 1] >= prior.minimum) * (
                prior_ranges_of_this_param[:, 0] <= prior.maximum
            )
        idxs_in_prior_range = np.arange(number_of_prior_ranges)[in_prior_range]
        return idxs_in_prior_range, dict(
            (param_name, prior_ranges[param_name][idxs_in_prior_range]) for param_name in param_names
        )
    ...
    def _update_basis(self, parameters=None):
        ...
        for basis_type, number_of_bases in zip(
            ...
            for param_name in self.weights[prior_range_key]:
                if param_name not in parameters:
                    continue
                in_prior_range *= (self.weights[prior_range_key][param_name][:, 0] <= parameters[param_name]) * (
        ...
        frequency_nodes, linear_indices, quadratic_indices = self._unique_frequency_nodes_and_inverse[
            basis_number_linear
    ...
    def calculate_snrs(self, waveform_polarizations, interferometer, return_array=True, parameters=None):
        ...
        for mode in waveform_polarizations["linear"]:
            response = interferometer.antenna_response(
                parameters["ra"], parameters["dec"], time_ref, parameters["psi"], mode
            )
        ...
        calib_factor = interferometer.calibration_model.get_calibration_factor(
            frequency_nodes, prefix=f"recalib_{interferometer.name}_", **parameters
        )
        h_linear *= calib_factor[linear_indices]
        h_quadratic *= calib_factor[quadratic_indices]

        optimal_snr_squared = np.vdot(
            np.abs(h_quadratic) ** 2, self.weights[interferometer.name + "_quadratic"][self.basis_number_quadratic]
        )

        dt = interferometer.time_delay_from_geocenter(parameters["ra"], parameters["dec"], time_ref)
        dt_geocent = parameters["geocent_time"] - interferometer.strain_data.start_time
        ifo_time = dt_geocent + dt

        indices, in_bounds = self._closest_time_indices(ifo_time, self.weights["time_samples"])
        if not in_bounds:
            ...
            d_inner_h_tc_array = np.einsum(
                "i,ji->j",
                np.conjugate(h_linear),
                self.weights[interferometer.name + "_linear"][self.basis_number_linear][indices],
            )

            d_inner_h = self._interp_five_samples(self.weights["time_samples"][indices], d_inner_h_tc_array, ifo_time)
    ...
    def _interp_five_samples(time_samples, values, time):
        ...
        """
        r1 = (-values[0] + 8.0 * values[1] - 14.0 * values[2] + 8.0 * values[3] - values[4]) / 4.0
        r2 = values[2] - 2.0 * values[3] + values[4]
    ...
    def _calculate_d_inner_h_array(self, times, h_linear, ifo_name):
        ...
        c = (a**3.0 - a) / 6.0
        d = (b**3.0 - b) / 6.0
        r1 = (-d_inner_h_m2 + 8.0 * d_inner_h_m1 - 14.0 * d_inner_h_0 + 8.0 * d_inner_h_p1 - d_inner_h_p2) / 4.0
        r2 = d_inner_h_0 - 2.0 * d_inner_h_p1 + d_inner_h_p2
    ...
    def perform_roq_params_check(self, ifo=None):
        ...
        if self.roq_params_check is False:
            ...
            if getattr(self, "roq_params_file", None) is not None:
                msg = f"Check ROQ params {self.roq_params_file} with roq_scale_factor={self.roq_scale_factor}"
            else:
        ...
        if ifo.maximum_frequency > roq_maximum_frequency:
            raise BilbyROQParamsRangeError(
                f"Requested maximum frequency {ifo.maximum_frequency} larger than ROQ basis "
                f"fhigh {roq_maximum_frequency}"
            )
        if ifo.minimum_frequency < roq_minimum_frequency:
            raise BilbyROQParamsRangeError(
                f"Requested minimum frequency {ifo.minimum_frequency} lower than ROQ basis flow {roq_minimum_frequency}"
            )
        if ifo.strain_data.duration != roq_segment_length:
            raise BilbyROQParamsRangeError("Requested duration differs from ROQ basis seglen")
        ...
        if roq_minimum_chirp_mass is not None:
            if priors.minimum_chirp_mass is None:
                ...
                raise BilbyROQParamsRangeError(
                    f"Prior minimum chirp mass {priors.minimum_chirp_mass} less than "
                    f"ROQ basis bound {roq_minimum_chirp_mass}"
                )
        ...
        if roq_maximum_chirp_mass is not None:
            if priors.maximum_chirp_mass is None:
                ...
                raise BilbyROQParamsRangeError(
                    f"Prior maximum chirp mass {priors.maximum_chirp_mass} greater than "
                    f"ROQ basis bound {roq_maximum_chirp_mass}"
                )
            ...
            for ifo in self.interferometers:
                if self.roq_params is not None:
                    ...
                    roq_frequencies = create_frequency_series(
                        sampling_frequency=roq_scaled_maximum_frequency * 2, duration=roq_scaled_segment_length
                    )
                    ...
                    overlap_frequencies, ifo_idxs_this_ifo, roq_idxs_this_ifo = np.intersect1d(
                        ifo.frequency_array[ifo.frequency_mask], roq_frequencies, return_indices=True
                    )
                else:
                    ...
                    if len(ifo_idxs_this_ifo) != len(roq_idxs_this_ifo):
                        raise ValueError(f"Mismatch between ROQ basis and frequency array for {ifo.name}")
                logger.info(
    ...
    def _set_weights_linear(self, linear_matrix, basis_idxs, roq_idxs, ifo_idxs):
        ...
        for basis_idx in basis_idxs:
            ...
            for ifo in self.interferometers:
                ifft_input[:] *= 0.0
                linear_weights = np.zeros((len(self.weights["time_samples"]), basis_size), dtype=complex)
                ...
                linear_weights *= 4.0 * number_of_time_samples / self.interferometers.duration
    ...
    def _set_weights_linear_multiband(self, linear_matrix, basis_idxs):
        ...
        for ifo in self.interferometers:
            ...
            for b, Tb in enumerate(Tbs):
                ...
                Db = np.fft.rfft(over_whitened_time_data[-int(2.0 * fhigh_basis * Tb) :])[
                    start_frequency_bin : end_frequency_bin + 1
                ]
                start_idx_of_next_band = start_idx_of_band + end_frequency_bin - start_frequency_bin + 1
                tc_shifted_data[ifo.name][start_idx_of_band:start_idx_of_next_band] = (
                    4.0
                    / Tb
                    * Db[:, None]
                    * np.exp(
                        2.0 * np.pi * 1j * fs[:, None] * (self.weights["time_samples"][None, :] - ifo.duration + Tb)
                    )
                )
    ...
    def _set_weights_quadratic_multiband(self, quadratic_matrix, basis_idxs):
        ...
        for ifo in self.interferometers:
            ...
            inverse_psd_frequency[np.arange(len(ifo.power_spectral_density_array))[ifo.frequency_mask]] = (
                1.0 / ifo.power_spectral_density_array[ifo.frequency_mask]
            )
            ...
            for b, Tb in enumerate(Tbs):
                ...
                start_idx_of_next_band = start_idx_of_band + end_frequency_bin - start_frequency_bin + 1
                multibanded_inverse_psd[ifo.name][start_idx_of_band:start_idx_of_next_band] = (
                    4.0
                    / Tb
                    * np.fft.rfft(
                ...
                for basis_type in ["linear", "quadratic"]:
                    ...
                    if key in self.weights:
                        ...
                        for param_name in self.weights[key]:
                            grp.create_dataset(param_name, data=self.weights[key][param_name])
                    key = f"frequency_nodes_{basis_type}"
                    if key in self.weights:
                        grp = f.create_group(key)
                        for i in range(len(self.weights[key])):
                            grp.create_dataset(str(i), data=self.weights[key][i])
                    for ifo in self.interferometers:
                        key = f"{ifo.name}_{basis_type}"
                        grp = f.create_group(key)
                        for i in range(len(self.weights[key])):
    ...
    def load_weights(self, filename, format=None):
        ...
        if format == "json":
            ...
            warnings.warn("json format for ROQ weights is deprecated, use hdf5 instead.", DeprecationWarning)
        ...
        if format == "npz":
            ...
            with h5py.File(filename, "r") as f:
                ...
                for basis_type in ["linear", "quadratic"]:
                    key = f"prior_range_{basis_type}"
                    if key in f:
                        idxs_in_prior_range, selected_prior_ranges = self._select_prior_ranges(f[key])
                        weights[key] = selected_prior_ranges
                    else:
                        idxs_in_prior_range = [0]
                    key = f"frequency_nodes_{basis_type}"
                    if key in f:
                        weights[key] = [f[key][str(i)][()] for i in idxs_in_prior_range]
                    for ifo in self.interferometers:
                        key = f"{ifo.name}_{basis_type}"
                        weights[key] = [f[key][str(i)][()] for i in idxs_in_prior_range]
        return weights
    ...
    def _get_time_resolution(self):
        ...
        def calc_fhigh(freq, psd, scaling=20.0):
            """
            ...
            integrand1 = np.power(freq, -7.0 / 3) / psd
            integral1 = simpson(y=integrand1, x=freq)
            integrand3 = np.power(freq, 2.0 / 3.0) / (psd * integral1)
    ...
    def generate_time_sample_from_marginalized_likelihood(self, signal_polarizations=None, parameters=None):
        ...
        for interferometer in self.interferometers:
            snrs += self.calculate_snrs(waveform_polarizations=signal_polarizations, interferometer=interferometer)
