...
class SymmetricLogUniform(Prior):
    ...
    def cdf(self, val, *, xp=np):
        ...
        # val = xp.atleast_1d(val)
        # norm = 0.5 / xp.log(self.maximum / self.minimum)
        # cdf = xp.zeros((len(val)))
        # lower_indices = xp.where(xp.logical_and(-self.maximum <= val, val <= -self.minimum))[0]
        # upper_indices = xp.where(xp.logical_and(self.minimum <= val, val <= self.maximum))[0]
        # cdf[lower_indices] = -norm * xp.log(-val[lower_indices] / self.maximum)
        # cdf[xp.where(xp.logical_and(-self.minimum < val, val < self.minimum))] = 0.5
        # cdf[upper_indices] = 0.5 + norm * xp.log(val[upper_indices] / self.minimum)
        # cdf[xp.where(self.maximum < val)] = 1
        # return cdf
...
class Beta(Prior):
    ...
    def cdf(self, val, *, xp=np):
        if isinstance(val, (float, int)):
            ...
            _cdf = xp.nan_to_num(btdtr(self.alpha, self.beta,
                                 (val - self.minimum) / (self.maximum - self.minimum)))
            _cdf *= val >= self.minimum
            _cdf *= val <= self.maximum
            _cdf += val > self.maximum
