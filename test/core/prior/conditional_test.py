...
class TestConditionalPriorDict(unittest.TestCase):
    ...
    def test_rescale(self):
        ...
        res = self.conditional_priors.rescale(keys=self.test_sample.keys(), theta=ref_variables)
    ...
    def test_cdf(self):
        ...
        self.assertEqual(
            self.conditional_priors.rescale(sample.keys(), self.conditional_priors.cdf(sample=sample).values()),
            list(sample.values()),
        )
    ...
    def test_combined_conditions(self):
        ...
        priors["a"] = bilby.core.prior.ConditionalUniform(condition_func=a_condition_func, minimum=0, maximum=1)

        priors["b"] = bilby.core.prior.LogUniform(minimum=1, maximum=10)

        priors["d"] = bilby.core.prior.ConditionalUniform(condition_func=d_condition_func, minimum=0.0, maximum=1.0)
