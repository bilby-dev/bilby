...
class TestCoupledTimeAndFrequencySeriesFromInit(unittest.TestCase):
    ...
    def test_time_array_correct_from_init(self):
        self.assertTrue(np.array_equal(self.test_series,
...
class TestSettingTimeSeries(unittest.TestCase):
    ...
    def setUp(self):
        ...
        duration = 2
        self.test_series = tupak.utils.CoupledTimeAndFrequencySeries(start_time=start_time,
    ...
    def test_change_time_series_sets_frequency_array(self):
        ...
        self.assertTrue(np.array_equal(self.test_series.frequency_series, self.expected_frequency_series))
...
class TestSettingFrequencySeries(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_series = tupak.utils.CoupledTimeAndFrequencySeries(start_time=self.expected_start_time,
                                                                     sampling_frequency=sampling_frequency,
                                                                     duration=duration)
        new_frequency_series = np.linspace(1, 100, 1000)
        self.expected_sampling_frequency, self.expected_duration = tupak.utils. \
            get_sampling_frequency_and_duration_from_time_array(new_frequency_series)
        self.expected_time_series = tupak.utils. \
            create_time_series(sampling_frequency=self.expected_sampling_frequency,
                               duration=self.expected_duration,
                               starting_time=self.expected_start_time)
        self.test_series.frequency_series = new_frequency_series
    ...
    def test_change_frequency_series_sets_start_time(self):
        self.assertEqual(self.expected_start_time, self.test_series.start_time)

    def test_change_frequency_series_sets_sampling_frequency(self):
        self.assertEqual(self.expected_sampling_frequency, self.test_series.sampling_frequency)

    def test_change_frequency_series_sets_duration(self):
        self.assertEqual(self.expected_duration, self.test_series.duration)

    def test_change_frequency_series_sets_time_array(self):
        self.assertTrue(np.array_equal(self.test_series.time_series, self.expected_frequency_series))
