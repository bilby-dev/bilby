...
class TestBasicGWTransient(unittest.TestCase):
    def setUp(self):
        ...
        self.interferometers.set_strain_data_from_power_spectral_densities(sampling_frequency=2048, duration=4)
    ...
    def test_noise_log_likelihood(self):
        ...
        self.assertAlmostEqual(-4014.1787704539474, self.likelihood.noise_log_likelihood(), 3)
...
class TestGWTransient(unittest.TestCase):
    ...
    def test_repr(self):
        expected = (
            f"GravitationalWaveTransient(interferometers={self.interferometers},\n\twaveform_generator={self.waveform_generator},\n\t"
            f"time_marginalization={False}, distance_marginalization={False}, phase_marginalization={False}, "
            f"calibration_marginalization={False}, priors={self.prior})"
        )
        self.assertEqual(expected, repr(self.likelihood))
    ...
    def test_interferometers_setting_interferometer_list(self):
        ifos = bilby.gw.detector.InterferometerList(
            [bilby.gw.detector.get_empty_interferometer(name=name) for name in ["H1", "L1"]]
        )
        self.likelihood.interferometers = ifos
        self.assertListEqual(bilby.gw.detector.InterferometerList(ifos), self.likelihood.interferometers)
        self.assertIsInstance(self.likelihood.interferometers, bilby.gw.detector.InterferometerList)
    ...
    def test_reference_frame_agrees_with_default(self):
        ...
        self.assertEqual(
            new_likelihood.log_likelihood_ratio(parameters), self.likelihood.log_likelihood_ratio(parameters)
        )
        new_likelihood.parameters.update(parameters)
        self.likelihood.parameters.update(parameters)
        self.assertEqual(new_likelihood.log_likelihood_ratio(), self.likelihood.log_likelihood_ratio())
...
class TestROQLikelihood(unittest.TestCase):
    def setUp(self):
        ...
        linear_matrix_file = f"{roq_dir}/B_linear.npy"
        quadratic_matrix_file = f"{roq_dir}/B_quadratic.npy"
        ...
        self.linear_matrix_file = f"{roq_dir}/B_linear.npy"
        self.quadratic_matrix_file = f"{roq_dir}/B_quadratic.npy"
        self.params_file = f"{roq_dir}/params.dat"
        ...
        ifos.inject_signal(parameters=self.test_parameters, waveform_generator=non_roq_wfg)
    ...
    def test_create_roq_weights_with_params(self):
        ...
        self.assertEqual(
            roq.log_likelihood_ratio(self.test_parameters), self.roq.log_likelihood_ratio(self.test_parameters)
        )
    ...
    def test_create_roq_weights_fails_with_max_frequency(self):
        ifos = bilby.gw.detector.InterferometerList(["H1"])
        ifos.set_strain_data_from_power_spectral_densities(sampling_frequency=2**14, duration=4)
    ...
    def test_create_roq_weights_fails_due_to_duration(self):
        ifos = bilby.gw.detector.InterferometerList(["H1"])
        ifos.set_strain_data_from_power_spectral_densities(sampling_frequency=self.sampling_frequency, duration=16)
        with self.assertRaises(BilbyROQParamsRangeError):
            bilby.gw.likelihood.ROQGravitationalWaveTransient(
                interferometers=ifos,
                waveform_generator=self.roq_wfg,
...
class TestRescaledROQLikelihood(unittest.TestCase):
    def test_rescaling(self):
        ...
        self.priors.pop("mass_2")
        # Testing is done with the 4s IMRPhenomPV2 ROQ basis
        self.priors["chirp_mass"] = bilby.core.prior.Uniform(12.299703 / scale_factor, 45 / scale_factor)
        self.priors["mass_ratio"] = bilby.core.prior.Uniform(0.125, 1)
        self.priors["geocent_time"] = bilby.core.prior.Uniform(1.19, 1.21)
...
class TestROQLikelihoodHDF5(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.priors["geocent_time"] = bilby.core.prior.Uniform(
            self.injection_parameters["geocent_time"] - 0.1, self.injection_parameters["geocent_time"] + 0.1
        )

    @parameterized.expand(
        [
            (_path_to_basis, 20.0, 2048.0, 16),
            (_path_to_basis, 10.0, 1024.0, 16),
            (_path_to_basis, 20.0, 1024.0, 32),
            (_path_to_basis_mb, 20.0, 2048.0, 16),
        ]
    )
    def test_fails_with_frequency_duration_mismatch(self, basis, minimum_frequency, maximum_frequency, duration):
        ...
        search_waveform_generator = bilby.gw.waveform_generator.WaveformGenerator(
            ...
            frequency_domain_source_model=bilby.gw.source.binary_black_hole_roq,
            waveform_arguments=dict(
                reference_frequency=self.reference_frequency, waveform_approximant=self.waveform_approximant
            ),
        )
        with self.assertRaises(BilbyROQParamsRangeError):
            bilby.gw.likelihood.ROQGravitationalWaveTransient(
    ...
    def test_number_of_loaded_bases(self, basis_linear, basis_quadratic, mc_range, roq_scale_factor):
        ...
        for ifo in interferometers:
            self.assertEqual(len(likelihood.weights[f"{ifo.name}_linear"]), number_of_bases_linear)
            self.assertEqual(len(likelihood.weights[f"{ifo.name}_quadratic"]), number_of_bases_quadratic)
    ...
    def assertLess_likelihood_errors(
        ...
        if add_cal_errors:
            ...
            for ifo in interferometers:
                ...
                for i in range(spline_calibration_nodes):
                    ...
                    self.injection_parameters[f"{prefix}amplitude_{i}"] = rng.normal(loc=0, scale=0.05)
                    self.injection_parameters[f"{prefix}phase_{i}"] = rng.normal(loc=0, scale=5 * np.pi / 180)
        ...
        likelihood = bilby.gw.GravitationalWaveTransient(
            interferometers=interferometers, waveform_generator=waveform_generator, priors=self.priors
        )
...
class TestCreateROQLikelihood(unittest.TestCase):
    ...
    def test_from_hdf5(self, basis_linear, basis_quadratic):
        ...
        search_waveform_generator = bilby.gw.waveform_generator.WaveformGenerator(
            ...
            frequency_domain_source_model=bilby.gw.source.binary_black_hole_roq,
            waveform_arguments=dict(reference_frequency=reference_frequency, waveform_approximant=waveform_approximant),
        )
    ...
    def test_from_npy(self, from_array):
        ...
        basis_linear = f"{roq_dir}/B_linear.npy"
        if from_array:
            basis_linear = np.load(basis_linear).T
        basis_quadratic = f"{roq_dir}/B_quadratic.npy"
        if from_array:
            basis_quadratic = np.load(basis_quadratic).T
        fnodes_linear = np.load(f"{roq_dir}/fnodes_linear.npy")
        fnodes_quadratic = np.load(f"{roq_dir}/fnodes_quadratic.npy")
        params_file = f"{roq_dir}/params.dat"
...
class TestMBLikelihood(unittest.TestCase):
    def setUp(self):
        ...
        self.ifos.set_strain_data_from_power_spectral_densities(
            sampling_frequency=self.sampling_frequency,
            duration=self.duration,
            start_time=self.test_parameters["geocent_time"] - self.duration + 2.0,
        )
        ...
        for ifo in self.ifos:
            ...
            for i in range(spline_calibration_nodes):
                ...
                self.calibration_parameters[f"recalib_{ifo.name}_amplitude_{i}"] = rng.normal(loc=0, scale=0.05)
                self.calibration_parameters[f"recalib_{ifo.name}_phase_{i}"] = rng.normal(loc=0, scale=5 * np.pi / 180)
    ...
    def test_matches_original_likelihood(
        ...
        wfg = bilby.gw.WaveformGenerator(
            ...
            sampling_frequency=self.sampling_frequency,
            frequency_domain_source_model=bilby.gw.source.lal_binary_black_hole,
            waveform_arguments=dict(reference_frequency=self.fmin, waveform_approximant=waveform_approximant),
        )
        ...
        likelihood = bilby.gw.likelihood.GravitationalWaveTransient(interferometers=self.ifos, waveform_generator=wfg)
        likelihood_mb = bilby.gw.likelihood.MBGravitationalWaveTransient(
            interferometers=self.ifos,
            waveform_generator=wfg_mb,
        ...
        self.assertLess(
            abs(likelihood.log_likelihood_ratio(parameters) - likelihood_mb.log_likelihood_ratio(parameters)), tolerance
        )
        likelihood.parameters.update(parameters)
        likelihood_mb.parameters.update(parameters)
        self.assertLess(abs(likelihood.log_likelihood_ratio() - likelihood_mb.log_likelihood_ratio()), tolerance)
    ...
    def test_reference_chirp_mass_from_prior(self):
        ...
        wfg_mb = bilby.gw.WaveformGenerator(
            ...
            sampling_frequency=self.sampling_frequency,
            frequency_domain_source_model=bilby.gw.source.binary_black_hole_frequency_sequence,
            waveform_arguments=dict(reference_frequency=self.fmin, waveform_approximant="IMRPhenomD"),
        )
        ...
        likelihood2 = bilby.gw.likelihood.MBGravitationalWaveTransient(
            interferometers=self.ifos, waveform_generator=wfg_mb, priors=self.priors.copy()
        )
    ...
    def test_no_reference_chirp_mass(self):
        ...
        with self.assertRaises(TypeError):
            bilby.gw.likelihood.MBGravitationalWaveTransient(interferometers=self.ifos, waveform_generator=wfg_mb)
