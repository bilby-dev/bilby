...
for i in range(Nevents):
    c0 = random.rng.normal(true_mu_c0, true_sigma_c0)
    c1 = random.rng.uniform(-1, 1)
    injection_parameters = dict(c0=c0, c1=c1)

    data = model(x, **injection_parameters) + random.rng.normal(0, sigma, N)
...
def hyper_prior(dataset, mu, sigma):
    return np.exp(-((dataset["c0"] - mu) ** 2) / (2 * sigma**2)) / (2 * np.pi * sigma**2) ** 0.5
